...
// We're going to use the Pagerank algorithm
// (http://en.wikipedia.org/wiki/Pagerank), specifically the iterative algorithm
// for calculating the rank of a page We're going to run 6 iterations of the
// propage step. Implement the corresponding code in CUDA.
...
constexpr float MAX_ULPS = 100;
constexpr int NUM_ITERATIONS = 6;

void host_graph_propagate(uint *graph_indices, uint *graph_edges,
                          float *graph_nodes_in, float *graph_nodes_out,
                          float *inv_edges_per_node, int num_nodes) {
  // for each node
...
double host_graph_iterate(uint *graph_indices, uint *graph_edges,
                          float *graph_nodes_in, float *graph_nodes_out,
                          float *inv_edges_per_node, int nr_iterations,
                          int num_nodes) {
...
void generateGraph(int num_nodes, int avg_edges,
...
void checkErrors(const std::vector<float> &h_gpu_node_values_output,
  ...
  EXPECT_EQ(h_gpu_node_values_output.size(), h_cpu_node_values_output.size())
      << "GPU & CPU Output Vector sizes are different";
  int num_errors = 0;
  ...
  for (uint i = 0; i < h_gpu_node_values_output.size(); i++) {
    ...
    if (AlmostEqualUlps(n, c, MAX_ULPS)) continue;

    // error case //
    num_errors++;

    if (num_errors < 10) {
      ADD_FAILURE() << "Mismatch at node " << i << std::endl
                    << "Expected " << c << " and got " << n << std::endl;
    } else {
  ...
  if (num_errors) {
    FAIL() << "There were errors, quitting" << std::endl;
  }
}

TEST(testQ2, testPageRank) {
  ...
  for (uint i = 0; i < 7; ++i) num_nodes.push_back(num_nodes.back() * 2);

  for (uint i = 2; i < 20; ++i) avg_edges.push_back(i);
  ...
  for (const uint edge : avg_edges) {
    ...
    for (const uint node : num_nodes) {
      ...
      double gpu_time = device_graph_iterate(
          h_graph_indices.data(), h_graph_edges.data(),
          h_node_values_input.data(), h_gpu_node_values_output.data(),
          h_inv_edges_per_node.data(), NUM_ITERATIONS, node, edge);

      // generate reference output
      host_graph_iterate(h_graph_indices.data(), h_graph_edges.data(),
                         h_node_values_input.data(),
                         h_cpu_node_values_output.data(),
                         h_inv_edges_per_node.data(), NUM_ITERATIONS, node);
      ...
      std::cout << std::setw(15) << std::fixed << std::setprecision(2)
                << totalBytes / (gpu_time / 1000.) / 1E9 << std::flush;
    }
